...
private:
    ...
    int count_node(const Leaf* head, int val)
    {
        if (!head)
            return 0;
        else if (val < head->data)
            return count_node(head->left, val);
        else if (val > head->data)
            return count_node(head->right, val);
        else
        {
            return head->count;
        }
...
size_t Tree::count(int val)
{
    return count_node(root, val);
}
Tree::Tree() :root(NULL) {}
...
void menu2_1()
{
    ...
    cout << "5. calculate the number of repetitions of an element " << endl;
        ...
        if (choice == 1)
        {
            ...
            while (flag2)
            {
                ...
                else if (ch == 5)
                {
                    cout << "value: ";
                    cin >> val;
                    cout << "the number of repetitions of an element: " << head.count(val);
                    cout << endl << "Press 'Backspace' if want to back" << endl << endl;
                    choi = _getch();
                    if (choi == 8) flag2 = true;
